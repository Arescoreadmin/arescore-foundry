...
from .services import SessionDispatchError, SessionService
...
class SessionResponse(BaseModel):
    ...
    def from_record(
        ...
        *,
        ...
        if include_plan and record.topology is not None:
...
def build_admin_router(*, service: SessionService) -> APIRouter:
    ...
    async def create_session(payload: SessionCreateRequest) -> SessionResponse:
        topology = Topology.from_yaml(payload.topology)
        try:
            record, plan = await service.create_session(
                name=payload.name,
                topology=topology,
            )
        except SessionDispatchError as exc:
    ...
    async def list_sessions() -> list[SessionResponse]:
        return [
            ...
            for record in service.list_sessions()
        ]
