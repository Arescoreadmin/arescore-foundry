...
from backend.orchestrator.app.messaging import InMemoryEventBus, SpawnEvent
...
class FailingEventBus(InMemoryEventBus):
    async def publish_spawn_event(self, event: SpawnEvent) -> None:  # type: ignore[override]
        raise RuntimeError("publish failed")


class DisconnectedEventBus(InMemoryEventBus):
    @property
    def is_connected(self) -> bool:  # type: ignore[override]
        return False


def test_session_creation_failure_returns_service_unavailable(tmp_path: Path) -> None:
    settings = Settings(session_store_path=str(tmp_path / "sessions.json"))
    store = SessionStore(tmp_path / "sessions.json")
    bus = FailingEventBus()
    app = create_app(settings=settings, event_bus=bus, session_store=store)
    ...
    body = response.json()
    assert "Failed to queue session" in body["detail"]

    records = store.list()
    assert len(records) == 1
    assert records[0].state == SessionState.FAILED


def test_session_creation_rejected_when_event_bus_disconnected(tmp_path: Path) -> None:
    settings = Settings(session_store_path=str(tmp_path / "sessions.json"))
    store = SessionStore(tmp_path / "sessions.json")
    bus = DisconnectedEventBus()
    app = create_app(settings=settings, event_bus=bus, session_store=store)
    client = TestClient(app)
    ...
    assert response.status_code == 503
    payload = response.json()
    assert "Event bus is not connected" in payload["detail"]
